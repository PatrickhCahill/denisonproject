...
%functions
const = 2*N/t^3 *(beta0/pi)^(3/2)*pi;

R = 54;

%p1 = @(x,y,z) const.*exp(-beta0.*(x.^2+y.^2+z.^2+R^2)./t^2).*besseli(0,(2*beta0/t^2).*(y.^2+z.^2)); %Boltzmann distribution
p = @(x,y,z) 2*N./(t.*sqrt(y.^2+z.^2)*beta0)*(beta0/pi)^(3/2).*exp((-beta0/t^2).*(x.^2+R^2+y.^2+z.^2)).*sinh((2*beta0*R/t^2).*sqrt(y.^2+z.^2));
